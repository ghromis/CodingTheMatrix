...
from GF2 import one
from vec import Vec
from hw4 import exchange, vec2rep
...
row_space_2 = [Vec({0,1,2,3},{0:1,1:4,2:0,3:0}),Vec({0,1,2,3},{0:0,1:2,2:2,3:0}),
               Vec({0,1,2,3},{0:0,1:0,2:1,3:1})]
col_space_2 = [Vec({0, 1, 2},{0:1,1:0,2:0}),Vec({0, 1, 2},{0:4,1:2,2:0}),
               Vec({0, 1, 2},{0:0,1:2,2:1})]
...
def subset_basis(T): 
    ...
    >>> subset_basis([a0,a1,a2,a3]) == [Vec({'c', 'b', 'a', 'd'},{'a': 1}),
    Vec({'c', 'b', 'a', 'd'},{'b': 1}), Vec({'c', 'b', 'a', 'd'},{'c': 1})]
    True
    '''
    L=[]
    for e in T:
        L.append(e)
        if my_is_independent(L) == False:
            L.remove(e)
    return L
...
def direct_sum_decompose(U_basis, V_basis, w):
    ...
    for e in range(len(U.D[1])):
        du[e] = W.f[e]
    WU = Vec(U.D[1], du)
    u = U*WU
    k=0
    for e in range(len(U.D[1]),len(U.D[1])+len(V.D[1])):
        dv[k] = W.f[e]
        k = +1
    WV = Vec(V.D[1], dv)
    v = V*WV
...
def find_matrix_inverse(A):
    ...
    >>> M = Mat(({0, 1, 2}, {0, 1, 2}), {(0, 1): one, (1, 2): 0, (0,
    0): 0, (2, 0): 0, (1, 0): one, (2, 2): one, (0, 2): 0, (2, 1): 0,
    (1, 1): 0})
    >>> find_matrix_inverse(M) == Mat(({0, 1, 2}, {0, 1,
    2}), {(0, 1): one, (2, 0): 0, (0, 0): 0, (2, 2): one, (1, 0): one,
    (1, 2): 0, (1, 1): 0, (2, 1): 0, (0, 2): 0})
    True
    '''
    L=[]
    for e in range(len(A.D[0])):
        d = {i:0 for i in range(len(A.D[0]))}
        d[e]=one
        v = Vec(A.D[0],d)
        b = solve(A, v)
        L.append(b)
